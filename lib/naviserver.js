...
var trackWriter = require('./tracks/trackWriter.js');
var fs = require('fs');
...
exports.startApp = function () {
  ...
  app.get('/track', function(req, res){
    ...
    var coordinates = fs.readFileSync('track_coordinates.txt','utf8');
    res.write(coordinates);
    res.write(']},"properties": {"time": [');
    var timestamps = fs.readFileSync('track_timestamps.txt','utf8');
  ...
  if(argv.writetrack) {
    trackWriter.createTrackWriter(streamSources.gaugeData.all.filter(function(msg){return msg.type === 'position'}));
  }
