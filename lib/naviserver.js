...
exports.startApp = function () {
  ...
  apiserver.registerHandlers(app);
  app.use(express.static(__dirname + '/../webapp/', { maxAge: oneDay }));
  var server = require('http').createServer(app);

  var ws = new (require('ws')).Server({ noServer: true });
  server.on('upgrade', function (req,socket,head) {
    ws.handleUpgrade(req, socket, head, function (websocket) {
      winston.info("Ws connect")
      connectionEventsAsIntegers.push(1);
      streamSources.treeData.subscribe(function(event) {
        try {
          websocket.send(JSON.stringify(event.value()));
        } catch (ex) {
          winston.info("Ws disconnect because of " + ex);
          connectionEventsAsIntegers.push(-1);
          return Bacon.noMore;
        }
      });
