...
StreamBundle.prototype = {
  ...
  createStreamWithConfiguration: function(configurationName) {
    var configurationFilename = './sourceconfiguration/' + this.name + '/' +
      configurationName.replace(/[^a-z0-9]/gi, '_').toLowerCase()
      + '.js';
    winston.info('Reading source configuration from ' + configurationFilename);
    return require(configurationFilename).createFromBundle(this);
  }
}
