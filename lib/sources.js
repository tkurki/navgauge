...
StreamBundle.prototype = {
  ...
  push: function (msg) {
    ...
    if (this.extractTypeFunction != undefined) {
      this.getTypeStream(this.extractTypeFunction(message)).push(message);
    }
  },
...
exports.usages = function () {
  return require('optimist')
    ...
    .usage('Example for raw nmea/n2k data from file: --nmea=file:cassiopeida.nmea.data.gz --n2k=file:cassiopeia.n2k.data.gz')
    .usage('Example for json combined data: --json=file:cassiopeida.json.gz')
}
