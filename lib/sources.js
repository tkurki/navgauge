...
StreamBundle.prototype = {
  ...
  createStreamWithConfiguration: function(configurationName, useNested) {
    ...
    var streamConfig;
    try {
      streamConfig = require(configurationFilename);
    } catch (error) {
      winston.info("Unable to load configuration " + configurationName);
      streamConfig = require('./sourceconfiguration/' + this.name + '/default.js');
    }
    var streamFromConfig = streamConfig.createFromBundle(this);
    return  useNested
