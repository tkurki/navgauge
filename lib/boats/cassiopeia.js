...
exports.boat = {
  configureStreams: function (streams) {
    ...
    streams.treeData.plug(streams.n2k.getTypeStream(n2kMessages.WATERDEPTH).map(function (msg) {
      return {
        id: 'navigation.depth',
    ...
    streams.treeData.plug(streams.nmea.json.filter(function (msg) {
      return msg.type === 'nav-info'
    }).flatMap(function (msg) {
        return Bacon.fromArray([
          {
    ...
    streams.treeData.plug(streams.n2k.getTypeStream(n2kMessages.WIND).flatMap(
      function(n2k) {
        return Bacon.fromArray([
          {
